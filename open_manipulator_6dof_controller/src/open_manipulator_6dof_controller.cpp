...
void OpenManipulatorController::moveitTimer(double present_time)
{
  ...
  if (moveit_plan_state_ == true)
  {
    ...
    if (path_time > moveit_sampling_time_)
    {
      ...
      // get the number of joints here. If they are more than 1 it means 
      // the planned path is for arm and if they are equal to 1, it means 
      // they are for gripper and for gripper, we need to call the goal_tool_ service. 
      // size_t __jsize = joint_trajectory_.points[0].positions.size()
      // log::warn("The length of joint is " << (int) __jsize);
...
{
  ros::init(argc, argv, "open_manipulator_6dof_controller");
